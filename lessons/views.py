...
def explain_section(request, lesson_id):
    ...
    if request.method == "POST":
        ...
        - Содан кейін сол сөйлемді қазақ тіліне қысқаша аудар бірақ "Қазақша" деп ескертіп айтудың керекгі жоқ
        ...
        if explanation_text:
            ...
            system_instructions = (
                "You are NOT allowed to add ANY extra words, greetings, confirmations, or meta comments. "
                "Your ONLY task is to read aloud EXACTLY the text provided between <<READ_START>> and <<READ_END>>. "
                "Do NOT say anything before or after. "
                "Do NOT acknowledge the instruction. "
                "Do NOT summarize, translate, or explain. "
                "Simply produce speech that corresponds 1:1 to the characters inside the markers. "
                "If the input is empty, stay completely silent."
            )
